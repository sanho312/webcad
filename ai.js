...
(function () {
  ...
  const SYSTEM = [
    ...
    '[커브] pt(x,y,z) · ptGrid(nx,ny,dx,dy)=점 격자 · line(a:점,b:점) · rect(c:점,w,h) · circle(c:점,r) · polygon(c,r,sides) · arc(c,r,a0,a1) · plineN(pts:점리스트,closed) · divide(crv,count)→출력0=점들·출력1=접선각(참조는 "노드id:1") · offsetC(crv,d)',
    ...
    '리스트 매칭(개체별 다른 값): move/rotate/extrude의 숫자 입력에 리스트(series/range/expr/rand)를 연결하면 개체마다 다른 값이 적용된다. 예: 트위스트 타워 = 층 사각형들 + rotate deg에 series(0,15,층수) / 계단식 지형 = geoIn 필지들 + extrude h에 rand·series / 파도 배열 = ptGrid + move dz에 expr(sin).',
