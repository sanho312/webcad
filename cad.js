...
function extrudeSetBase(px, py) {
  ...
  ex.anchorPy = py; ex.h0 = (ex.srf && ex.applied) ? ex.val : 0; ex.heightPhase = 'awaitTop';
  if (!ex.applied) extrudeApplyKind(); // extrudecrv/평면면: 이 클릭에서 비로소 입체 생성
  extrudeSetVal(ex.h0 || 10); // 면 밀당은 현재 높이에서, 그 외는 0(→10)에서 시작
  extrudeRefresh();
...
function extrudeValidSel(cmd) {
  ...
  // extrudesrf: 밀거나 당길 솔리드(기둥·벽) 또는 솔리드로 만들 평면 곡선/원 — 폴리라인은 closed 무관 3점 이상
  return sel.filter(e => (e.bim && ['wall', 'column'].includes(e.bim.kind)) || (!e.bim && ((e.type === 'LWPOLYLINE' && (e.points || []).length >= 3) || e.type === 'CIRCLE')));
}
...
function extrudeStart(cmd, sel) {
  ...
  if (srf) { // 면 밀당: 기존 솔리드는 현재 높이로 그대로 보이게, 화면 클릭/숫자로 높이 조절 (스냅)
    ...
    extrudeRefresh();
    logLine(`  ▷ extrudesrf: 면을 밀거나 당길 높이 — 화면을 클릭해 마우스로 조절(다른 객체에 스냅) 또는 높이값 입력 · Esc`, 'info');
