...
function renderScene(isActive) {
  ...
  for (const s of v3.solids) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      faces.push({ pts: quad, d: (quad[0][2] + quad[1][2] + quad[2][2] + quad[3][2]) / 4, color: s.color, shade: 0.55 + 0.45 * lightA, glass: s.glass, eid: s.eid, rf: s.rf, fk: 'side', fi: i });
    }
  ...
  if (typeof extrudePend !== 'undefined' && extrudePend && extrudePend.srf && v3.srfHi && v3.srfHi.size) {
    ...
    for (const s of v3.solids) {
      ...
      const top = s.poly.map((p, i) => proj3D(p[0], p[1], zt[i]));
      const bot = s.poly.map(p => proj3D(p[0], p[1], s.z0));
      // 강조 대상: 옆면 모드=클릭한 옆면 사각, 아랫면 모드=아랫면, 기본=윗면
...
function pick3DAt(px, py, additive) {
  ...
  v3.pickFace = hit ? { eid: hit.eid, fk: hit.fk || null, fi: hit.fi != null ? hit.fi : null } : null; // 마지막 클릭 면(top/bot/side + 옆면이면 변 인덱스) — extrudesrf 아랫면·옆면 밀당용
...
function extrudeStart(cmd, sel) {
  ...
  if (pf && pf.fk === 'side' && sel.length === 1) {
    ...
    } else if (e.bim && e.bim.kind === 'wall') {
      logLine('  ⚠ 벽의 옆면 밀당은 아직 미지원 — 이동(move)이나 속성 패널의 두께(t)로 조절하세요. 윗면 기준으로 진행합니다.', 'warn');
    }
  }
