...
function renderScene(isActive) {
  ...
  for (const s of v3.solids) {
    ...
    for (let i = 0; i < n; i++) {
      ...
      let sSh, sSh3 = null;
      if (s.glow) sSh = 1;
      else if (v3.lighting) { sSh = litFace(mx, my, midz, onx, ony, 0, false); if (LIT_RGB[3]) sSh3 = [LIT_RGB[0], LIT_RGB[1], LIT_RGB[2]]; } // 색번짐
...
function litFace(wx, wy, wz, nx, ny, nz, twoSided) {
  ...
  if (B) for (const g of B) {
    ...
    const k = d / g.range;
    const e = dot * g.power * 1.35 / (1 + k * k);
    tr += e * g.cr; tg += e * g.cg; tb += e * g.cb;
  }
  // 채널별 밝기 = 공통(환경광+직접광) + 그 채널의 반사광
  const cl = v => Math.max(0.05, Math.min(1.5, v));
